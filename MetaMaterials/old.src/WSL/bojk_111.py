...
path_ = OSP.dirname(pathlib.Path(__file__).resolve())
...
def find_all_dirs_backend(file):
	...
	if os.path.isfile(file):
	...
	elif os.path.isfile(pathlib.Path(file).resolve()) : 
		...
		#return (False , pathlib.Path(file).resolve())
...
def solely_check_file_type(file):
	...
	return os.path.isfile(pathlib.Path(file).resolve())

def find_all_dirs(path):

	#global main_list 

	main_list = { pathlib.Path(path).resolve() }
...
def sbf_search_engine(path , query : str  , case_sensitivity : bool  = False , full_path: bool = False ):
	...
	if len(xx_) > 0 :
		...
		main_list.update( { ( pathlib.Path(path).resolve() , max(xx_ , key = lambda x9: x9[-1])[-1] ) } ) 
...
def arguments():
	...
	parser.add_argument('-g' , '--generate' , '--create' , dest = 'first_action' , action = 'store_const' , const = 'gen' , default = None , 
	...
	parser.add_argument('-p' , '--pushd' ,
						...
						help = 'Push the directory on the shell stack, i.e. the 𝓅𝓊𝓈𝒽𝒹 command')
...
def config_print(yaml_file = None , configuration_ = 'all' ):
	...
	if configuration_ == "ALL":
		...
		logging.info(f"BOJK Program File: [{pathlib.Path(__file__).resolve() }]")
...
def driver_main(parser : Namespace ):
	...
	if parser.first_action == 'version' :
		...
		logging.info('Generating YAML Like Configuration File' , 'green')
		...
		logging.critical("Standard Blank YAML like File generated by the name of {yaml_file}")
